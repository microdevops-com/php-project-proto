...
build: ## build environment and initialize composer and project dependencies
	docker build .docker/php$(PHP_VER)-fpm/ -t local/$(CI_PROJECT_PATH)/php$(PHP_VER)-fpm:master --build-arg CI_COMMIT_REF_SLUG=$(CI_COMMIT_REF_SLUG) --build-arg CI_SERVER_HOST=$(CI_SERVER_HOST) --build-arg CI_PROJECT_PATH=$(CI_PROJECT_PATH) --build-arg PHP_VER=$(PHP_VER)
	docker build .docker/php$(PHP_VER)-composer/ -t local/$(CI_PROJECT_PATH)/php$(PHP_VER)-composer:master ${DOCKER_BUILD_ARGS} --build-arg CI_COMMIT_REF_SLUG=$(CI_COMMIT_REF_SLUG) --build-arg CI_SERVER_HOST=$(CI_SERVER_HOST) --build-arg CI_PROJECT_PATH=$(CI_PROJECT_PATH) --build-arg PHP_VER=$(PHP_VER)
	docker build .docker/php$(PHP_VER)-dev/ -t local/$(CI_PROJECT_PATH)/php$(PHP_VER)-dev:master ${DOCKER_BUILD_ARGS} --build-arg CI_COMMIT_REF_SLUG=$(CI_COMMIT_REF_SLUG) --build-arg CI_SERVER_HOST=$(CI_SERVER_HOST) --build-arg CI_PROJECT_PATH=$(CI_PROJECT_PATH) --build-arg PHP_VER=$(PHP_VER) --build-arg USER_ID=$(shell id -u) --build-arg GROUP_ID=$(shell id -g)
